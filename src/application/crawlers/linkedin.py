...
class LinkedInCrawler(BaseSeleniumCrawler):
    """
    A web scraper for LinkedIn using Selenium and BeautifulSoup.

    This class provides methods to log in (deprecated), extract profile details, and scrape posts.

    Attributes:
        model (PostDocument): The data model for storing scraped posts.
    """
    model = PostDocument

    def __init__(self, scroll_limit: int = 5, is_deprecated: bool = True) -> None:
        """
        Initializes the LinkedInCrawler with optional scroll limit and deprecation flag.

        Args:
            scroll_limit (int): Number of times to scroll down when scraping. Default is 5.
            is_deprecated (bool): Flag to disable deprecated methods. Default is True.
        """
    ...
    def set_extra_driver_options(self, options) -> None:
        """Sets additional driver options for Selenium."""
        options.add_experimental_option("detach", True)

    def login(self) -> None:
        """
        Logs into LinkedIn. (Deprecated)

        Raises:
            DeprecationWarning: If method is called.
            ImproperlyConfigured: If credentials are missing.
        """
    ...
    def extract(self, link: str, **kwargs) -> None:
        """
        Extracts data from a LinkedIn profile page.

        Args:
            link (str): The LinkedIn profile URL.
            **kwargs: Additional arguments, expects 'user' with user details.
        """
        if self._is_deprecated:
            raise DeprecationWarning(
        ...
        # Scrape different sections of the profile
        data = {  # noqa
        ...
        # Click 'Show More' on the profile page to load full posts
